...
export default function ImageGrid({ images, title }: ImageGridProps) {
  ...
  useEffect(() => {
    ...
    if (!images || !Array.isArray(images)) {
      console.error('ImageGrid received invalid images:', images);
      setHasImages(false);
      return;
    }
    ...
    const validImages = images.filter(img => {
      ...
      return img && img.url && typeof img.url === 'string'; // Make sure image objects have a valid URL
    });
    ...
    validImages.forEach((img, index) => {
      if (index < 5) { // Log just the first few to avoid console spam
        console.log(`Image ${index}:`, typeof img === 'string' ? img : `${img.source || 'unknown'} - ${img.url}`);
      }
    });
  ...
  }, [images]); // Remove inView dependency to ensure layout is generated regardless of viewport
  ...
  }, [inView, images, hasImages, generateBentoLayout]);
          ...
          >
            {cell.src.startsWith('https://') ? (
              ...
              <img
                ...
                onError={(e) => {
                  console.error(`Failed to load external image: ${cell.src}`, e);
