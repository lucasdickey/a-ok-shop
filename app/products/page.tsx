...
import { getAllProducts, ShopifyProduct } from '@/app/lib/shopify';
...
export default async function ProductsPage({
  ...
  const filteredProducts = products.filter(product => {
    ...
    if (category && product.productType.toLowerCase() !== category.toLowerCase()) {
      return false;
    }
    ...
    <div className="container py-8">
      ...
      {error ? (
        <div className="bg-red-50 border border-red-200 text-red-800 rounded-lg p-4 mb-8">
          ...
          <p>{error}</p>
          <p className="mt-2 text-sm">Please check your Shopify API configuration and try again.</p>
        </div>
      ) : null}
