...
export default function RunHumansRun({
  ...
  useEffect(() => {
    ...
    // Game constants - adjusted for MacBook Pro 12" screen
    const GRID_SIZE = 20;
    ...
    const draw = () => {
      try {
        ...
        // Draw score and lives IN THE CANVAS
        ctx.fillStyle = "#FFFFFF";
        ...
        // Draw lives
        ctx.fillText(`LIVES: ${livesRef.current}`, canvas.width - 120, 25);
    ...
    // Check collisions - FIXED to prevent game reset bug
    const checkCollisions = () => {
      try {
        ...
        // Check token collisions - FIXED logic to prevent game reset
        if (tokens) {
          for (let i = tokens.length - 1; i >= 0; i--) {
            const token = tokens[i];
            if (token && token.x === player.x && token.y === player.y) {
              // Token collection logic - properly increment without resetting
              tokens.splice(i, 1);
              player.tailLength += 1;
              ...
              // Check win condition - only trigger modal, don't reset game
              if (newTokens >= TOKENS_TO_WIN) {
                setGameWon(true);
                setShowWinModal(true);
                if (onGameComplete) onGameComplete(true);
                return; // Exit game loop when won
              }
  ...
  }, [gameStarted, onGameComplete, setGameOver, setGameWon, setLives, setScore, setTokensCollected, score]);
    ...
    <>
      ...
      {!gameStarted && (
        <div className="flex flex-col items-center justify-center w-full max-w-[700px] mx-auto relative">
          <div
            className="flex flex-col items-center justify-center p-8 w-full h-[500px] relative overflow-hidden"
            style={{
          ...
          >
            ...
            <div className="relative z-10 flex flex-col items-center">
              <h2 className="text-4xl mb-16 text-center font-bold text-white drop-shadow-lg">
                RUN, HUMAN, RUN!
              </h2>
              <div className="mb-8 text-center text-white drop-shadow-lg font-bold">
                <p className="mb-3 inline-block px-3 py-2 rounded bg-black bg-opacity-50 text-sm">
              ...
              <button
                ...
                className="px-6 py-3 mt-8 bg-green-500 text-white font-bold rounded shadow-lg"
              >
                START GAME
              </button>
            </div>
